...
T-TEST PAIRS=g3 WITH g1 (PAIRED)
 /CRITERIA=CI(.95).
""",
12: """CROSSTABS /TABLES=sex BY pass10
 /STATISTICS=CHISQ PHI
 /CELLS=COUNT EXPECTED ROW SRESID.
""",
13: """GRAPH /SCATTERPLOT(BIVAR)=g3 WITH g1.
...
""",
14: """DESCRIPTIVES VARIABLES=g1 g3
 /STATISTICS=MEAN STDDEV MIN MAX.
...
CORRELATIONS /VARIABLES=g1 g3.
* Duyarlilik kontrolu; sifirlar otomatik eksik sayilmaz.
TEMPORARY.
SELECT IF (g3>0).
...
def opening(number,frame):
    text=f"""* Baslangicta SPSS calisma dizini kitap projesinin koku olmalidir.
GET DATA /TYPE=XLSX
...
def main():
    ...
    for d in ['raw','excel','csv','syntax','sav','results']:
    ...
    for i in range(1,15):
        ...
        d.to_csv(OUT/'csv'/f'b{i:02d}.csv',index=False)
        (OUT/'syntax'/f'open-b{i:02d}.sps').write_text(opening(i,d),encoding='utf-8')
        syntax=f"INSERT FILE='companion/spss/syntax/open-b{i:02d}.sps'\n ERROR=STOP.\n"+ANALYSES[i]
    ...
    pd.DataFrame(mapping).to_csv(OUT/'chapter-map.csv',index=False)
    V={}; tables={}
    ...
    for f in sorted(OUT.rglob('*')):
        if f.is_file() and f.suffix in ['.xlsx','.csv','.sps','.tex','.json'] and f.name!='manifest.csv':
